...
async function applyQualitySetting(quality) {
  ...
  if (typeof pub.setSubscriptionSettings === 'function') {
    ...
    try {
      ...
      // لا تعتمد فقط على enum — اطلب أبعاد 1080p كـ fallback أيضًا
      if (quality === 'high' && typeof pub.setVideoDimensions === 'function') {
        await pub.setVideoDimensions({ width: 1920, height: 1080 });
      }
      return;
    } catch {}
  }

  // بديل: setVideoDimensions بأبعاد تقريبية
  if (typeof pub.setVideoDimensions === 'function') {
    const dims = quality === 'low'
      ? { width: 320, height: 180 }
      : quality === 'high'
      ? { width: 1920, height: 1080 } // ← طلب 1080p
